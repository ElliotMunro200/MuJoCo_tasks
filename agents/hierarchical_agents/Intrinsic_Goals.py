...
class IG(Base_Agent):
    ...
    def __init__(self, config):
        ...
        self.max_sub_policy_timesteps = config.hyperparameters["LOWER_LEVEL"]["max_lower_level_timesteps"] # =5
        ...
        self.higher_level_agent = IG_Higher_Level_Goal_Selecting_Agent(self.higher_level_agent_config, self.lower_level_agent.actor_local)
...
class IG_Higher_Level_Goal_Selecting_Agent(DDPG):
    """Selects goals for the lower-level IG agent"""

    def __init__(self, config, lower_level_policy):
        super(IG_Higher_Level_Goal_Selecting_Agent, self).__init__(config)
        self.lower_level_policy = lower_level_policy
        self.num_intrinsic_goals = 4
        self.coords = {goal: [] for goal in range(self.num_intrinsic_goals)}
        self.rad_intrinsic_goals = 1
        self.make_goals()
        #self.number_goal_candidates = config.hyperparameters["number_goal_candidates"]

    # intrinsic goal selection [named to be compatible with DDPG.step()]
    def make_goals(self):
        angles = list(np.array(range(self.num_intrinsic_goals))*360/self.num_intrinsic_goals)
        for goal, angle in enumerate(angles):
            self.coords[goal] = np.array(np.sin(angle),np.cos(angle))*self.rad_intrinsic_goals

    def pick_action(self):
        choice = random.randint(0,self.num_intrinsic_goals-1)
        goal = self.coords[choice]
        return goal
    ...
    #                 goal = IG.goal_transition(state_obs, goal, next_state)
...
class Higher_Level_Agent_Environment_Wrapper(Wrapper):
    ...
    level agent"""
    def __init__(self, env, IG_agent):
        Wrapper.__init__(self, env)
        self.env = env
        self.IG_agent = IG_agent
        self.action_space = self.observation_space


    def reset(self, **kwargs):
        self.IG_agent.higher_level_state = self.env.reset(**kwargs)
        return self.IG_agent.higher_level_state

    def step(self, goal):
        self.IG_agent.higher_level_reward = 0
        self.IG_agent.step_lower_level_states = []
        self.IG_agent.step_lower_level_action_seen = []

        self.IG_agent.goal = goal
        self.IG_agent.lower_level_agent.episode_number = 0 # must reset lower level agent to 0 episodes completed otherwise won't run more episodes
        self.IG_agent.lower_level_agent.run_n_episodes(num_episodes=1, show_whether_achieved_goal=False, save_and_print_results=False)

        self.IG_agent.save_higher_level_experience()

        return self.IG_agent.higher_level_next_state, self.IG_agent.higher_level_reward, self.IG_agent.higher_level_done, {}
...
class Lower_Level_Agent_Environment_Wrapper(Wrapper):
    ...
    def __init__(self, env, IG_agent, max_sub_policy_timesteps):
        Wrapper.__init__(self, env)
        self.env = env
        self.meta_agent = IG_agent
    ...
    def update_goal(self, next_state):
        ...
        self.meta_agent.goal = IG.goal_transition(self.meta_agent.higher_level_state, self.meta_agent.goal,
                                                    next_state)
    ...
    def calculate_intrinsic_reward(self, internal_state, internal_next_state, goal):
        ...
        # truncate the state to the goal-relevant portion (normally x-y).
        trunc_state = self.trunc(internal_state)
        trunc_next_state = self.trunc(internal_next_state)

        desired_trunc_next_state = trunc_state + goal
        error = desired_trunc_next_state - trunc_next_state
        # some pos and neg errors. want all as neg as int_reward.
